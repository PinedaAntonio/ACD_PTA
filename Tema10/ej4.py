...
# Identificación y tratamiento de outliers (método Z-score)
for col in ["AskPrice", "kmDriven"]:
    z_scores = (df[col] - df[col].mean()) / df[col].std()  # Calcular Z-score
    df = df[
        np.abs(z_scores) < 3
    ]  # Mantener solo valores dentro de 3 desviaciones estándar
...
# Establecer el límite superior del eje Y en 4.5 * 10^6
y_max_custom = 4.5 * 1e6

# Gráfica de predicción vs datos reales con índices reales, manteniendo el rango deseado del eje Y
...
plt.ylim(0, y_max_custom)  # Establecer el mismo rango del eje Y que la imagen
